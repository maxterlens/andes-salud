...
define([
    ...
    function procesarCaja(caja, resultado) {
        ...
        reversarCierresPrevios(datos.todosLosCierres, resultado, datos.encabezado, null);
     ...
     * @param {string|number|null} numeroMovimiento - Número de movimiento opcional para contexto de errores
     */
    function reversarCierresPrevios(todosLosCierres, resultado, encabezado, numeroMovimiento = null) {
    ...
    function procesarMovimiento(
        ...
        let customerId = movimiento.IdPaciente
        ...
        if (typeof customerId === "object" && customerId?.[0]) {
            customerId = customerId[0].value; // En caso de que el mapping retorne un array de resultados, tomar el primero
        }
        if (!customerId) {
        ...
        if (esEdicion) {
            ...
            movimiento.boletasEmitidas.forEach((boleta) => {
                try {
                    // ✅ AN-259: CC19 - Validar si la boleta está anulada
                    // Valores esperados: "S" = Anulada, "N" = Vigente
                    const boletaNula = utils.esSi(boleta.boletaNula);
                    
                    if (boletaNula) {
                        // ✅ CC19: Si está anulada, registrar sin procesamiento de forma de pago
                        nLog.audit("procesarMovimiento - Boleta Anulada", "Folio: " + boleta.folioBoleta + " Registrando como anulada (boletaNula='S'), sin aplicación de forma de pago");
                    }
                    ...
                    if (utils.esSi(boleta.Anticipo)) {
                        ...
                        invoiceId = InvoiceDAO.createInvoice({
                            ...
                            boletaNula: boletaNula, // Valor de boletaNula para identificar en campo personalizado de la transaccion
        ...
        if (movimiento.NCEmitidas && movimiento.NCEmitidas.length > 0) {
            movimiento.NCEmitidas.forEach((nc) => {
                try {
                    let boletaData;
                    if (nc.anticipo === "S") {
                        boletaData = MappingDAO.getItemMapping({ categoria: getFlow("BoletasEmitidasAnticipo") });
                    } else {
                        boletaData = MappingDAO.getItemMapping({ categoria: getFlow("BoletasEmitidas") });
                    }

                    const subsidiariaId = getSubsidiaria(utils.formatearRut(nc.rutReceptor));
                    const ncId = CreditMemoDAO.createCreditMemo({
                        account: boletaData.cuentaCobrarBoleta,
                        customerId: customerId,
                        fechaTransaccion: fechaTransaccion,
                        folioNC: nc.folioNC,
                        ...
                        creditoItem: boletaData.articuloBoleta,
                        cuentaPaciente: movimiento.CuentaPaciente,
                    ...
                    if (nc.folioRef) {
                        try {
                            CreditMemoDAO.aplicarNCContraFactura(ncId, nc.folioRef, nc.montoTotal);
        ...
        if (movimiento.formaPago && movimiento.formaPago.length > 0) {
                ...
                try {
                    ...
                    if (pago.tipo && pago.tipo.includes("NC")) {
                        ...
                        if (esIntercompany || ["COPAGO", "REDONDEO", "DEVOLUCION"].includes(pago.tipoDocPago)) {
                            ...
                            /**
                             * ✅ AN-274: enrrutar por JE cuando hay mismatch de cliente en NCR misma subsidiaria  
                             */

                            // Consultar cliente de boleta a pagar
                            const invoiceEntity = InvoiceDAO.getInvoiceEntityByFolio(pago.folioDoc);

                            // Consultar cliente de NC
                            const creditmemoEntity = CreditMemoDAO.getCreditMemoEntityByFolio(pago.folioTipoPago);

                            // Validar si clientes de ambas transacciones coinciden
                            if (invoiceEntity === creditmemoEntity) {
                                // Si coinciden -> Aplicacion nativa
    
                                // ES MISMA SUBSIDIARIA Y PAGA UNA FACTURA (BOEE/BOAE) -> APLICACIÓN NATIVA DIRECTA (No lleva Journal)
                                CreditMemoDAO.aplicarNCComoFormaPago(pago.folioTipoPago, pago.folioDoc, pago.montoPago);
                            } else {
                                // Si difieren -> aplicacion por JE

                                // Recuperar cuenta contable de forma de pago para debito
                                const { cuentaContableDebito: ncCuentaDebito } = MappingDAO.getItemMapping({
                                    categoria: getFlow("FormaPago"),
                                    formaPagoTipo: pago.tipo.trim()
                                });
                                
                                // Recuperar cuenta por cobrar de boleta para credito
                                const cuentaCredito = MappingDAO.getItemMapping({ categoria: getFlow("BoletasEmitidas") }).cuentaCobrarBoleta;

                                // Construir lineas contables de JE
                                consolidado[claveConsolidacion].lineasEstandar.push({
                                    account: ncCuentaDebito, // Cuenta de forma de pago
                                    debit: parseFloat(pago.montoPago),
                                    entity: creditmemoEntity, // ID de cliente NC
                                    folio: pago.folioTipoPago,
                                    memo: `Forma Pago NC - ${pago.tipo}`,
                                    subsidiaria: subsidiariaDestino
                                });
                                consolidado[claveConsolidacion].lineasEstandar.push({
                                    account: cuentaCredito, // Cuenta por cobrar boleta
                                    credit: parseFloat(pago.montoPago),
                                    entity: invoiceEntity, // ID de cliente boleta a pagar
                                    folio: pago.folioDoc,
                                    memo: `Aplicacion NC - ${pago.tipoDocPago}`,
                                    subsidiaria: subsidiariaDestino
                                });

                                // Agregar lineas construidas a ncsParaAplicarAJournal
                                ncsParaAplicarAJournal.push({
                                    folioNC: pago.folioTipoPago,
                                    claveConsolidacion: claveConsolidacion,
                                    montoPago: pago.montoPago
                                ...
                                nLog.audit("procesarMovimiento - NC enrutada por mismatch de cliente", {
                                    folioNC: pago.folioTipoPago,
                                    folioDestino: pago.folioDoc,
                                    monto: pago.montoPago,
                                    clienteNC: creditmemoEntity,
                                    clienteDestino: invoiceEntity,
                                    subsidiaria: subsidiariaDestino,
                                    ruta: "JOURNAL_ENTRY"
                                });
                            }
            ...
            if (ncsParaAplicarAJournal.length > 0) {
                nLog.debug("procesarMovimiento - ncsParaAplicarAJournal", {
                    ncsParaAplicarAJournal: ncsParaAplicarAJournal
                });
                ncsParaAplicarAJournal.forEach((ncData) => {
                    try {
                        const jeId = mapJournalIds[String(ncData.claveConsolidacion)];
                        nLog.debug("procesarMovimiento - jeId", {
                            jeId: jeId
                        });

                        if (jeId) {
                            // Se utiliza esta función para aplicar la NC contra la línea de Journal que acabamos de crear
                            CreditMemoDAO.aplicarNCIntercompany(ncData.folioNC, jeId, ncData.montoPago);
                            nLog.debug("NC Aplicada a Journal exitosamente", `NC: ${ncData.folioNC} -> JE: ${jeId}, Monto: ${ncData.montoPago}`);
                        }
                    } catch (error) {
        ...
        const transaccionesCreadas = searchTransactionByMovementNumber({
            caja: unidadCaja,
            fechaCaja: fechaTransaccion,
            aperturaCaja: aperturaCaja,
            razonSocialCaja: razonSocialCaja,
        ...
        nLog.debug("procesarMovimiento - transaccionesCreadas", {
            extension: transaccionesCreadas.length,
        ...
        if (movimiento.MovimientoAnulado === "N" && facturasCreadas.length > 0) {
            nLog.audit("validacionBoletas - movimiento", {
            ...
            nLog.audit("validacionBoletas - facturasCreadas", {
                facturasCreadas: facturasCreadas
            });
            facturasCreadas.forEach((factura) => {
                try {
                    const saldo = InvoiceDAO.verificarSaldoPendiente(factura.id);
                    nLog.audit("validacionBoletas - saldo", {
                        movimiento: movimiento.numeroMovimiento,
                        folio: factura.folio,
                        saldo: saldo.saldoPendiente
                    });

                    // ✅ AN-259: CC19 - Agregar soporte para boleta anulada
                    if (saldo.saldoPendiente > 0 && !factura.boletaNula ) {
                    ...
                    } else if (saldo.saldoPendiente > 0 && factura.boletaNula ) {
                        nLog.audit("validacionBoletas - boleta nula", "Folio: " + factura.folio + " - Total: " +  saldo.montoTotal + " - Saldo pendiente: " +  saldo.saldoPendiente);
                    } else {
    ...
    function limpiarTransaccionesMovimiento(datos, numeroMovimiento) {
        ...
        try {
            ...
            nLog.debug("limpiarTransaccionesMovimiento - transacciones", {
                extension: transacciones.length,
                transacciones: transacciones
            });

            let idsExistentes = {};
            let transaccionesUnicas = transacciones.filter(function(item) {
                if (idsExistentes[item.id]) return false;
            ...
            nLog.debug("limpiarTransaccionesMovimiento - transaccionesOrdenadas", {
                extension: transaccionesOrdenadas.length,
                transaccionesOrdenadas: transaccionesOrdenadas
            });
            nLog.debug("limpiarTransaccionesMovimiento - transaccionesUnicas", {
                extension: transaccionesUnicas.length,
                transaccionesUnicas: transaccionesUnicas
            });
            ...
            //     }
            // });
            transaccionesUnicas.forEach((transaccion) => {
                const eliminada = deleteTransaction(transaccion.id, transaccion.recordtype);
                if (eliminada) {
                    resumen.eliminadas.push({ id: transaccion.id, tipo: transaccion.recordtype });
                } else {
