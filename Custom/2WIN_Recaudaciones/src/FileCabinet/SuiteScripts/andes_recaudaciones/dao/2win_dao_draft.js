define(["N/query", "N/record", "N/log"], function (query, record, nLog) {
    ...
    /**
     * Desaplica una transaccion
     * @param {number} internalidTransaction - ID de la transaccion
     * @param {string} transactionType - Tipo de la transaccion
     * @param {number} appliedId - ID de la transaccion a desaplicar
     */
    const unapplyTransaction = (internalidTransaction, transactionType, appliedId) => {
        try {
            nLog.debug("unapplyTransaction - parametros", {
                internalidTransaction: internalidTransaction,
            ...
            const transactionRecord = record.load({
                ...
                id: internalidTransaction,
                isDynamic: true
            });

            nLog.debug("unapplyTransaction - transactionRecord", {
                transactionRecord: transactionRecord
            });

            const lineCount = transactionRecord.getLineCount({ sublistId: "apply" });
            nLog.debug("unapplyTransaction - lineCount", lineCount);
            let unapplied = false;


            if (lineCount !== -1) {          
                for (let i = 0; i < lineCount; i++) {
                    const docId = transactionRecord.getSublistValue({ sublistId: "apply", fieldId: "doc", line: i });
                    nLog.debug("unapplyTransaction - docId", {
                        docId: Number(docId),
                        appliedId: Number(appliedId)
    
                    });
    
                    if (Number(docId) === Number(appliedId)) {
                        transactionRecord.selectLine({ sublistId: "apply", line: i });
                        transactionRecord.setCurrentSublistValue({ sublistId: "apply", fieldId: "apply", value: false });
                        transactionRecord.commitLine({ sublistId: "apply" });
                        unapplied = true;
                        break;
                    }
                }
            ...
            if (unapplied) {
                ...
                nLog.debug("unapplyTransaction", `Transaccion ${internalidTransaction} desaplicada del Invoice ${appliedId}`);
            } else {
                nLog.debug("unapplyTransaction", `No se encontró aplicación de Transaccion ${internalidTransaction} al Invoice ${appliedId}`);
            }
        } catch (e) {
            nLog.lineCount("unapplyTransaction - error", e);
            throw e;
        }
    };
    ...
    const deleteTransactionById = (transactionId, type) => {
        try {
            const appliedNCs = query
                .runSuiteQL({
                    query: `
                        ...
                        AppliedTx.recordtype AS applied_in_record_type,
            ...
            nLog.debug("deleteTransactionById - appliedNCs", {
                extension: appliedNCs.length,
                appliedNCs: appliedNCs
            });
            ...
            if (appliedNCs && appliedNCs.length > 0) {
                ...
                appliedNCs.forEach((nc) => {
                    try {
                        if (nc.applied_in_record_type && nc.applied_in_id) {
                            unapplyTransaction(nc.applied_in_id, nc.applied_in_record_type, transactionId);
                        };
                    } catch (error) {
                        nLog.error("deleteTransactionById - error", "Error desaplicando transaccion: " + nc.applied_in_type_name + " id: " + nc.applied_in_id)
                        resumen.fallidas.push({
                            transaccion: nc.recordtype,
                            id: nc.id,
                            error: error
                        });

                        resumen.errores += " " + error;
                    }
                });

                if (resumen.fallidas.length > 0) {
                    nLog.error("deleteTransactionById - error", {
                        resumen: resumen
                    });
                    throw new Error("Error en desaplicacion, fallo desaplicacion de " + resumen.fallidas.length + " transacciones, Errores: " + resumen.errores);
                };
            }
        } catch (e) {
