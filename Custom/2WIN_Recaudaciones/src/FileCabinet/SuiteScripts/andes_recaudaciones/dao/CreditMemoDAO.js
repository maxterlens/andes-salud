...
define(["N/record", "N/search", "N/log", "N/query", "./SalesOrderDAO"], function (record, search, nLog, query, SalesOrderDAO) {
    function createCreditMemo(data) {
        try {
            ...
            if (data.customerId && data.cuentaPaciente) {
                salesOrderFields = SalesOrderDAO.getSalesOrderFields(data.customerId, data.razonSocialCobro, data.cuentaPaciente);
            }
            if (data.esAnticipo === "N") {
                ...
                newRecord.setValue({ fieldId: "account", value: invoiceRecord.getValue("account") });
                ...
                for (let i = 0; i < itemCount; i++) {
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "item", line: i }))
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "quantity", line: i }))
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "rate", line: i }))
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "amount", line: i }))
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "tax1code", line: i }))
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "tax1amt", line: i }))
                    ...
                    if (invoiceRecord.getSublistValue({ sublistId: "item", fieldId: "description", line: i }))
                ...
                if (data.account) newRecord.setValue({ fieldId: "account", value: data.account });
            }
    ...
    function applyCreditMemo(cmId, invoiceId, prevunapply, amountToApply) {
        try {
            ...
            const invoiceRecord = search.lookupFields({
                type: "transaction",
                id: invoiceId,
                columns: [
                    "entity", 
                    "subsidiary", 
                    "type" // ✅ AN-274 - Agregar columna type para validar tipo posteriormente
                ]
            });
            nLog.debug("applyCreditMemo - invoiceRecord", { invoiceRecord: invoiceRecord });

            /**
             * ✅ AN-274 - Validar que el cliente de la transaccion coincida con el cliente de la NC antes de aplicar
             * Si no coincide *Solo si es Journal Entry -> ajustar la entidad de la NC para que coincida con la entidad de la transaccion 
             */  
            const isJournalEntry = invoiceRecord.type && invoiceRecord.type[0] && invoiceRecord.type[0].value === "Journal";  
            if (
                !isJournalEntry && // No es Journal Entry
                invoiceRecord.entity && 
                invoiceRecord.entity.length > 0 &&  
                invoiceRecord.entity[0].value // Se recupero valor para entity de transaccion destino
            ) {  
                const cmEntity = cmRecord.getValue("entity");  
                const invoiceEntity = invoiceRecord.entity[0].value; 

                // Si el entity no coincide, lanzar error para evitar aplicar
                if (invoiceEntity !== cmEntity) {  
                    throw new Error("NC: " + cmId + " pertenece al cliente: " + cmEntity + ", el documento destino: " + invoiceId + " pertenece al cliente: " + invoiceEntity + ", No se puede cambiar el cliente de una NC existente.");
                }  
            }; 
            ...
            for (let i = 0; i < lineCount; i++) {
                ...
                if (Number(docId) === Number(invoiceId)) {
                    ...
                    // Setear el monto a aplicar si se proporciona
                    if (amountToApply) {
                        cmRecord.setCurrentSublistValue({ 
                            sublistId: "apply", 
                            fieldId: "amount", 
                            value: parseFloat(amountToApply) 
                        });
                        nLog.audit("CreditMemoDAO", `Aplicando monto específico: ${amountToApply} a Invoice ${invoiceId}`);
                    }
                    cmRecord.commitLine({ sublistId: "apply" });
                    applied = true;
                    break;
     ...
     * @param {string} amountToApply - Monto a aplicar (opcional)
     */
    function aplicarNCContraFactura(cmId, folioRef, amountToApply) {
        try {
            const results = query
                .runSuiteQL({
                    query: `select top 1
            ...
            if (results && results.length > 0) {
                ...
                applyCreditMemo(cmId, invoiceId, false, amountToApply);
            } else {
     ...
     * @param {string} amountToApply - Monto a aplicar (opcional)
     */
    function aplicarNCComoFormaPago(folioNC, folioFactura, amountToApply) {
        try {
            nLog.debug("aplicarNCComoFormaPago", { folioNC: folioNC, folioFactura: folioFactura, amountToApply: amountToApply });
            const results = query
                .runSuiteQL({
                    query: `select top 1
                    transaction.id,
            ...
            if (results && results.length > 0) {
                ...
                aplicarNCContraFactura(ncId, folioFactura, amountToApply);
     ...
     * @param {string} amountToApply - Monto a aplicar (opcional)
     */
    function aplicarNCIntercompany(folioNC, idJournalEntry, amountToApply) {
        try {
            nLog.debug("aplicarNCIntercompany", { folioNC: folioNC, idJournalEntry: idJournalEntry, amountToApply: amountToApply });
            ...
            // Aplicar la NC a la factura con el monto específico
            applyCreditMemo(ncId, idJournalEntry, false, amountToApply);
    ...
    /**
     * Funcion para recuperar el internalid de un cliente a partir del folio de la transaccion
     * @param {string} folio - Folio de la transaccion
     * @returns internalid del cliente
     */
    function getCreditMemoEntityByFolio(folio) {
        try {
            // nLog.debug("getCreditMemoEntityByFolio - parametro", {
            //     folio: folio
            // });
            ...
            nLog.debug("getCreditMemoEntityByFolio - results", {
            ...
            if (results && results.length > 0) {
                ...
                throw new Error(`No se encontró creditmemo con folio ${folio}`);
            };
        } catch (e) {
            nLog.error("getCreditMemoEntityByFolio - error", e);
            throw e;
        }
    }
    ...
    return {
        ...
        getCreditMemoEntityByFolio: getCreditMemoEntityByFolio
    };
});
