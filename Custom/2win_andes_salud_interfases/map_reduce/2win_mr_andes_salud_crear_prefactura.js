    ...
    function (runtime, task, file, nLog, dom_prefactura, dao_prefactura_queue, dao_file) {
        ...
        function getInputData() {
            ...
            try {
                ...
                // Obtener registros pendientes de la cola para SEND^IN
                const registrosPendientes = dao_prefactura_queue.getPending(50, "SEND^IN");
        ...
        function map(context) {
            ...
            try {
                ...
                contenido.data.forEach(function(data) {
                    ...
                    // Crear prefactura
                    const id_prefactura = dom_prefactura.crear(data);
                    nLog.audit("map - id_prefactura creada", id_prefactura);
        ...
        function summarize(summary) {
            ...
            Object.keys(resultados).forEach(function(queueRecordId) {
                ...
                if (hayErrores) {
                    ...
                    if (registrosNoProcesados.length > 0) {
                        const queueRecord = dao_prefactura_queue.getPending(1, "SEND^IN").find(function(r) { return r.id == queueRecordId; });
                        if (queueRecord) {
                            const archivo = dao_file.cargarArchivo(queueRecord.archivoId);
                            const contenido = JSON.parse(archivo.contenido);
                            ...
                            const notificacion = {
                                ...
                                estado: "error",
                                codigo: 400,
                                mensaje: "Proceso de creación de prefacturas finalizado con errores",
                                tipo_proceso: "Creación Prefactura",
                    ...
                    if (queueRecord) {
                        ...
                        const notificacion = {
                            ...
                            estado: "success",
                            codigo: 200,
                            mensaje: "Proceso de creación de prefacturas finalizado correctamente",
                            tipo_proceso: "Creación Prefactura",
                            id_proceso: queueRecord.uuid,
                            data: registros
                        };
            ...
            const pendientes = dao_prefactura_queue.getPending(1, "SEND^IN");
