/**
 * @NApiVersion 2.1
 * @module ./2win_dao_nota_credito.js
 * @NModuleScope Public
 */
define(["./2win_dao", "N/search", "N/log", "N/record", "../lib/2win_lib_peticion", "../lib/moment"], function (dao, search, nLog, record, libPeticion, moment) {
    /**
     * Busca una Nota de Crédito por su internalId.
    ...
    function busquedaRegistroPorId(internalId) {
        try {
            ...
            const objSearch = {
                type: "customrecord_2win_nota_credito",
                ...
                columns: [
                    ...
                    search.createColumn({ name: "custrecord_2win_nota_credito_numero", label: "numero" }),
                    search.createColumn({ name: "custrecord_2win_nota_credito_tipo", label: "tipo" })
                ]
            };
            ...
            if (result.length > 0) {
                ...
                throw new Error(`No se encontró Nota de Crédito para id: ${internalId}`);
            }
        } catch (error) {
    ...
    /**
     * Recupera los campos relevantes de una Nota de Crédito a partir del registro cargado.
     * @param {record.Record} registro
     * @returns {Object} objeto con los campos de la nota de crédito
     */
    function recuperarCamposRegistro(registro) {
        try {
            ...
            const numero = registro.getValue({ fieldId: "custrecord_2win_nota_credito_numero" });
            const tipo = registro.getValue({ fieldId: "custrecord_2win_nota_credito_tipo" });
            ...
            nLog.debug("recuperarCamposRegistro - notaCredito", notaCredito);
            return notaCredito;
        } catch (error) {
    ...
    /**
     * Definir campos de registro de Nota de Crédito.
        ...
        try {
            ...
            for (let campo in parametro.datos) {
                ...
                if (Array.isArray(parametro.datos[campo])) {
                    ...
                    if (campo === "externalid") {
                        registro.setValue({ fieldId: "custrecord_2win_nota_credito_externalid", value: parametro.datos[campo] });
                        ...
                        registro.setValue({ fieldId: "custrecord_2win_nota_credito_amount", value: parametro.datos[campo] });
                    } else if (campo === "numero") {
                        registro.setValue({ fieldId: "custrecord_2win_nota_credito_numero", value: parametro.datos[campo] });
                    } else if (campo === "tipo") {
                        registro.setValue({ fieldId: "custrecord_2win_nota_credito_tipo", value: parametro.datos[campo] });
                    } else {
    ...
    function crearRegistro(parametro) {
        try {
            ...
            // Crear registro
            let registro = record.create({ type: "customrecord_2win_nota_credito", isDynamic: true });
    ...
    /**
     * Editar registro existente de Nota de Crédito.
     * @param {object} parametro - Datos para los campos del registro.
     * @return {object} - Datos generados en la ejecucion.
     */
    function editarRegistro(parametro) {
        try {
            ...
            // Cargar registro
            let registro = record.load({ type: "customrecord_2win_nota_credito", id: parametro.idRegistroNetsuite, isDynamic: true });
    ...
    /**
     * Eliminar registro de Nota de Crédito.
    ...
    function eliminarRegistro(idRegistro) {
        try {
            ...
            return { success: true, message: "Registro de Nota de Crédito eliminado correctamente." };
