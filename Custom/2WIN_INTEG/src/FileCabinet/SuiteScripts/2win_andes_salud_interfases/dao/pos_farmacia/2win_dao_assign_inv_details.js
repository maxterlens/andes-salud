...
define(['N/log', 'N/search', 'N/query', './2win_dao_search_item', 'N/runtime'],
    function(log, search, query, daoSearchItem, runtime) {
        ...
        function _asignarInventario(invDet, itemId, locationId, qtyNeeded, isSerial, isLot, trxType, isDynamic) {
            ...
            // var itemPlaceholders  = itemChunk.map(function()  { return '?'; }).join(',');
            // var locPlaceholders   = locChunk.map(function()   { return '?'; }).join(',');
            ...
            var rows = query.runSuiteQL({
                query: [
                    ...
                    '  ON ib.item = ail.item AND ib.location = ail.location',
                    'INNER JOIN InventoryNumberLocation inl',
                    '  ON ib.inventorynumber = inl.inventorynumber AND ib.location = inl.location',
                    'WHERE ib.item     IN (' + itemId + ')',
                    '  AND ib.location IN (' + locationId  + ')',
                    '  AND ib.location IN (SELECT id FROM location WHERE isinactive = \'F\')',
                    '  AND ib.quantityavailable  > 0',
                    '  AND inl.quantityavailable > 0',
                    '  AND (NVL(ail.quantityonhand, 0) - NVL(ail.quantitycommitted, 0)) > 0',
                    '  AND ib.inventorystatus NOT IN (',
                    '        SELECT id FROM InventoryStatus',
                    '        WHERE name IN (\'Bloqueado\', \'En Inspección\', \'Damaged\')',
                    '      )',
                    'ORDER BY ib.item, ib.location, ib.lastmodifieddate ASC'
                ].join(' '),
                params: []   // primero los items, luego las locations
            }).asMappedResults();
