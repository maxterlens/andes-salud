    ...
    function (runtime, task, file, nLog, dom_prefactura, dao_prefactura_queue, dao_file) {
        ...
        function getInputData() {
            ...
            try {
                ...
                // Obtener registros pendientes de la cola para SEND^UPD
                const registrosPendientes = dao_prefactura_queue.getPending(50, "SEND^UPD");
        ...
        function map(context) {
            ...
            try {
                ...
                contenido.data.forEach(function(data) {
                    ...
                    // Editar prefactura
                    const id_prefactura = dom_prefactura.editar(data);
                    nLog.audit("map - id_prefactura editada", id_prefactura);
        ...
        function summarize(summary) {
            ...
            Object.keys(resultados).forEach(function(queueRecordId) {
                ...
                if (hayErrores) {
                    ...
                    if (registrosNoProcesados.length > 0) {
                        const queueRecord = dao_prefactura_queue.getPending(1, "SEND^UPD").find(function(r) { return r.id == queueRecordId; });
                        if (queueRecord) {
                            const archivo = dao_file.cargarArchivo(queueRecord.archivoId);
                            const contenido = JSON.parse(archivo.contenido);
                            ...
                            const notificacion = {
                                ...
                                estado: "error",
                                codigo: 400,
                                mensaje: "Proceso de edición de prefacturas finalizado con errores",
                                tipo_proceso: "Edición Prefactura",
                    ...
                    if (queueRecord) {
                        ...
                        const notificacion = {
                            ...
                            estado: "success",
                            codigo: 200,
                            mensaje: "Proceso de edición de prefacturas finalizado correctamente",
                            tipo_proceso: "Edición Prefactura",
                            id_proceso: queueRecord.uuid,
                            data: registros
                        };
            ...
            const pendientes = dao_prefactura_queue.getPending(1, "SEND^UPD");
