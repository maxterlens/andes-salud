...
define([
...
], function (libAuditoria, libCustodia, daoCliente, daoOrdenVenta, nLog, runtime, util, daoProducto, libFormato, daoFarmacia) {
    ...
    const crearOrdenConsumo = (parametro) => {
        try {
            ...
            if (isConsumo) {
                ...
                const lineasConError = [];
                const lineasExitosas = [];
                const identificadoresProcesados = new Set();
                ...
                detalleConsumo.forEach((e) => {
                    try {
                        ...
                        // Validar duplicidad de línea antes de validación de stock
                        if (crgCorrel) {
                            if (Farmacia.existeLineaPermanente(crgCorrel)) {
                                throw new Error(`Ya existe una línea permanente con el identificador ${crgCorrel}`);
                            }
                            if (identificadoresProcesados.has(String(crgCorrel))) {
                                throw new Error(`Identificador duplicado dentro del consumo ${crgCorrel}`);
                            }
                            identificadoresProcesados.add(String(crgCorrel));
                        }
    ...
    const devolverConsumo = (parametro) => {
        try {
            ...
            let { tipoMensaje, identificadorUnicoPaciente, numeroFicha, numeroIngreso, numeroCuentaPaciente, devolucionMedicamentos } = parametro[0];
            nLog.debug("devolverConsumo - devolucionMedicamentos", devolucionMedicamentos);
            if (!util.isArray(devolucionMedicamentos)) devolucionMedicamentos = [devolucionMedicamentos];
            custodia.custrecord_2win_as_tiempo_proceso = Date.now();
            ...
            if (lineasConError.length > 0) {
                ...
                payload.Cargos = lineasExitosas.map((l) => l.identificadorUnicoFila);
                payload.SinStock = [];
