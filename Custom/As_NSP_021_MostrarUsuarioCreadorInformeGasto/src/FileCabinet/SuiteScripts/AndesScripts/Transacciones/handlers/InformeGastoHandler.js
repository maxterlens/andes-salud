...
define([
    'N/runtime',
    '../repositories/InformeGastoRepository.js'
], (runtime, InformeGastoRepository) => {

    function agregarCreadoPor(scriptContext) {
        let { form, newRecord, type } = scriptContext;

		log.error('values', {type} );
        if (type == 'view' || type == 'edit') {
            let createdById = InformeGastoRepository.getCreatedBy(newRecord.id);
			log.error('createdById', createdById);
            ...
            createdByField.defaultValue = createdById;
            form.insertField({
                field : createdByField,
                nextfield : 'expensereportcurrency'
            });

			let newField = newRecord.getField('custbody_as_created_by');
			log.error('newfield', newField);
		}
    }
